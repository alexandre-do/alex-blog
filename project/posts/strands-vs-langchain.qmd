...
So this post is the version of that answer I wish I could just link to. Each section below starts with the underlying problem every agentic framework has to solve, then walks through **LangChain's answer**, then **Strands' answer** — rather than a feature checklist. At the end there's a heuristic you can apply in about ninety seconds.

## First: what are we even comparing?

Every agent framework has to answer the same question — who writes the reason → act → observe loop, and at what altitude do you plug in? LangChain and Strands answer it very differently.
...
Strands Agents, by contrast, is a single SDK from AWS, in Python and TypeScript. You get an `Agent`, and you get first-class multi-agent orchestrators — `Swarm`, `Graph`, `Workflow` — plus A2A for talking to remote agents. One install, one mental model.

So "Strands vs LangChain" is really "a three-layer stack where you choose your altitude vs one opinionated SDK." That framing already tells you something: if you want to build an unusual control flow and have the runtime not fight you, LangGraph is closer to that. If you want to be handed a working multi-agent topology, Strands is closer to that.

Everything below is downstream of that split.

## Axis 1: the agent loop and how you bound it

Both run the same loop — call the model, execute any tool calls, feed results back, repeat until the model stops asking for tools. Nobody is innovating on the loop itself. The difference is in the controls around it, and how you find out why a run stopped.

### LangChain: bound the loop with middleware

LangChain's answer is middleware — less about naming the exit, more about composing behaviour on the way there:
...
Underneath, LangGraph has a `recursion_limit` (default 1000 super-steps) as the ultimate backstop. It's a blunter instrument than a turn budget — it counts graph steps, not model turns — but it does stop runaway loops.

### Strands: named stop reasons
...
**The takeaway on this axis:** LangChain gives you better answers to "how do I change what happens on the way?"; Strands gives you better answers to "why did this stop?" Both matter, but they matter to different people. If you're building the agent and need retries, fallbacks, and context management, middleware is worth more. If you're operating agents and debugging production incidents, named stop reasons are worth a lot.
